...
def run_train(args, hparams):
    ...
    nsteps = 0
    def configure_scheduler(nsteps):
        ...
        if args.lrsched is None:
            scheduler = learning_rates.WarmupThenReduceLROnPlateau(
                ...
                verbose=True,)
        elif args.lrsched == 'linear':
            from transformers import get_linear_schedule_with_warmup
            scheduler = get_linear_schedule_with_warmup(optimizer, hparams.learning_rate_warmup_steps, nsteps)
        return scheduler
    ...
    run = wandb.init(project="self-attentive-parser", reinit=True)
    ...
    global ntrbatches
    ntrbatches = len(data_loader)
    training_steps = 0
    if args.max_steps:
        training_steps = args.max_steps
    elif args.max_epochs:
        training_steps = args.max_epochs*ntrbatches
  
    scheduler = configure_scheduler(nsteps = training_steps)
    ...
    def check_dev():
        ...
        print(f'total time to parse dev: {time.time() - dev_start_time}')
#        import ipdb; ipdb.set_trace()
        ...
        wandb.log({"LF": current_dev_fscore})
        # if dev_fscore.fscore > best_dev_fscore:
        #     best_dev_fscore = dev_fscore.fscore
        

        if dev_fscore.fscore > best_dev_fscore and args.nruns == 1:
            ...
            best_dev_fscore = dev_fscore.fscore
            wandb.log({"best LF": best_dev_fscore})
        ...
        elif dev_fscore.fscore > best_dev_fscore:
            best_dev_fscore = dev_fscore.fscore
            wandb.log({"best LF": best_dev_fscore})
            best_dev_processed = total_processed
        ...
        for batch_num, batch in enumerate(data_loader, start=1):
            ...
            if current_processed >= check_every:
                ...
                if args.lrsched is None and ((math.isnan(current_dev_fscore) and best_dev_fscore >= 10)
                      or current_dev_fscore <= 0.8*best_dev_fscore):
...
def run_test(args):
    ...
    # print(f'memory: {peak_memory_usage}')
    if args.output_path == "-":
...
def main():
    ...
    subparser.add_argument("--postinit", default=False)
    subparser.add_argument("--mode", type=str, default=None, choices=["bce", "mlr", "normalized-bce"])
    ...
    # subparser.add_argument("--step-decay-factor", type=float, default=0.5)
    # subparser.add_argument("--attention-dropout", type=float, default=0.2)
    ...
    # subparser.add_argument("--test-path", default="data/ctb_5.1/ctb.test")
    ...
    if args.nruns > 1:
        ...
        grid = {
            ...
            'learning_rate': [5e-5, 6e-5], #1e-4],
            'weight_decay': [0.01],
            # 'learning_rate_warmup_steps': [160, 1000, 2000],
            # 'batch_size': [32, 64, 128],
            # 'lrsched': ['linear'],
            # 'max_steps': [11000, 15000]
            # 'step_decay_factor':[0.6,0.8]
        }
        import itertools
        if args.gseed < 0:
            keys = list(grid.keys())
